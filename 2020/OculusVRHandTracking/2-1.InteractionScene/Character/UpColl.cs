...
public class UpColl : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            else if (header.statAnim == AnimState.LYING)
            {
                header.headerCanvas.ShowText(11, Random.Range(0, 2));
                header.currentAI = header.StartCoroutine(header.Tingle());
                //header.LikeChange(20);
            }
            else
            {
                header.Stop();
                switch (header.statLike)
                {
                    ...
                    case LikeState.NORMAL:
                        header.PlayTriggerAnimation(1);
                        break;
                    case LikeState.FRIEND:
                        header.PlayTriggerAnimation(1);
                        break;
                }
                header.headerCanvas.ShowText(2, 0);
            }

            header.LikeChange(10);
            header.StartCoroutine(header.AfterTouch(2));
        }
        ...
        if (other.CompareTag("Brush"))
        {
            ...
            Debug.Log("UpBrush!");
            if (header.statAnim == AnimState.LYING)
            {
                int rand = Random.Range(0, 2);
                header.headerCanvas.ShowText(10, rand);
                header.currentAI = header.StartCoroutine(header.Tingle());
            }
            else if (other.GetComponent<Brush>().brushCount > 2)
            {
                header.AI_Move(6);
