...
public class MeshCut
{
    ...
    public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial)
    {
        ...
        Object.Destroy(leftSideObj.GetComponent<Collider>());

        MeshCollider leftMeshColl = leftSideObj.AddComponent<MeshCollider>();
        leftMeshColl.convex = true;

        leftSideObj.GetComponent<OVRGrabbable>().SetGrabbable(leftSideObj.transform, new Collider[1] { leftMeshColl });
        ...
        MeshCollider rightMeshColl =  rightSideObj.AddComponent<MeshCollider>();
        rightMeshColl.convex = true;

        Rigidbody rightRb = rightSideObj.AddComponent<Rigidbody>();

        OVRGrabbable rightGrabber = rightSideObj.AddComponent<OVRGrabbable>();
        rightGrabber.SetGrabbable(rightSideObj.transform, new Collider[1] { rightMeshColl });

        Sliceable rightSlice = rightSideObj.AddComponent<Sliceable>();
        rightSlice.sliceMat = capMaterial;
