...
*/
	...
	GameObject AddSource( int _index, string _name = "")
	{
        ...
        TableInfo_fx kTableInfo_fx = CGameTable.Instance.Get_TableInfo_fx( _index );
		if(kTableInfo_fx == null ) {
            //Debug.Log("ERROR: CGameFx AddSource Get_TableInfo_fx: " + _index);
			return null;
		}

        string prefab_name = "";
        if(_name != "")     prefab_name = "fx/" + _name;       //테이블없이 등록. 1000 이상으로하자.
        else                prefab_name = "fx/" + kTableInfo_fx.sResource;
	...
	public GameObject PlayFx(int _index, Vector3 pos, float _lifetime )
	{
        ...
        if (_renderer != null)
            fxCloneGO.GetComponent<Renderer>().sharedMaterial.renderQueue = 3000;// GetComponentInParent<UIPanel>().startingRenderQueue;
