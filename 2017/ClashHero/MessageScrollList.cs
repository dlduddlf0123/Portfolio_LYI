...
[System.Serializable]
public class MessageScrollItem
{
    public long uid;
    public int item_index;  //아이템 .
    public string desc;     //설명
    public string time;     //시간.  
    public int state;       //수령가능:0, 수령후 1

}

/*
public MessageScrollList kMessageScroll;
void Start()
{
    kMessageScroll.Setup(OnEvent_MessageScroll, "");
}
void OnEvent_MessageScroll(long _uid, string _order)
{
...
public class MessageScrollList : MonoBehaviour
{
    ...
    public List<MessageScrollItem> itemList;
    public Transform contentPanel;
    public GameObject element;
    MessageScrollObjectPool ObjectPool;
    ...
    private void Awake()
    {
        ObjectPool = gameObject.AddComponent<MessageScrollObjectPool>();
    ...
    public void Setup(EventCallback _callback, string _mode)
    {
        ...
        //add item
        //MessageScrollItem item1 = new MessageScrollItem(); item1.uid = 1; item1.item_index = 80001; item1.desc = "test item"; item1.time = "00:00"; item1.state = 0;
        //itemList.Add(item1);

        //foreach (MessageInfo item in kPlayer.MessageList)
        //{
        //    MessageScrollItem mitem = new MessageScrollItem();
        //    mitem.uid = item.uid;
        //    mitem.item_index = item.reward_index;
        //    mitem.desc = item.text;
        //    mitem.time = item.regist_time;
        //    mitem.state = 0;
        //    itemList.Add(mitem);
        //}
    ...
    void Sort()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            MessageScrollItem item = new MessageScrollItem();
            item.uid = uid;
            //item.price = uid;
    ...
    private void AddButtons()
    {
        ...
        {
            MessageScrollItem item = itemList[i];

            GameObject newButton = ObjectPool.GetObject(); //Pool에서 가져온다.
            newButton.transform.SetParent(contentPanel);
            newButton.transform.localScale = new Vector3(1, 1, 1);
            MessageScrollElement element = newButton.GetComponent<MessageScrollElement>();
            element.Setup(item, this); // 초기화.
        }
    }

    public void TryTransferItemToOtherShop(MessageScrollElement item)
    {
/*
...
*/
    ...
    void AddItem(MessageScrollItem itemToAdd, MessageScrollList shopList)
    {
        shopList.itemList.Add(itemToAdd);
    }

    private void RemoveItem(MessageScrollItem itemToRemove, MessageScrollList shopList)
    {
...
public class MessageScrollObjectPool : MonoBehaviour
{
    ...
    public GameObject GetObject()
    {
        ...
        else
        {
            ...
            MessageScrollObject_pool pooledObject = spawnedGameObject.AddComponent<MessageScrollObject_pool>();
    ...
    {
        MessageScrollObject_pool pooledObject = toReturn.GetComponent<MessageScrollObject_pool>();
...
public class MessageScrollObject_pool : MonoBehaviour
{
    public MessageScrollObjectPool pool;
}
